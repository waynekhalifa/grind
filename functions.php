...
if ( ! function_exists( 'grind_setup' ) ) :
	...
	function grind_setup() {
		 ...
		 * If you're building a theme based on grind, use a find and replace
		 * to change 'grind' to the name of your theme in all the template files.
		 */
		load_theme_textdomain( 'grind', get_template_directory() . '/languages' );
		...
		register_nav_menus( array(
			'menu-1' => esc_html__( 'Primary', 'grind' ),
		) );
...
function grind_content_width() {
	...
	$GLOBALS['content_width'] = apply_filters( 'grind_content_width', 640 );
}
...
function grind_scripts() {
	...
	wp_enqueue_script( 'grind-navigation', get_template_directory_uri() . '/js/navigation.js', array(), '20151215', true );

	wp_enqueue_script( 'grind-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20151215', true );
